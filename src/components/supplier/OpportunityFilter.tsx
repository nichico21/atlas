...
export default function OpportunityFilter({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      ...
      <div className={`transition-all duration-300 overflow-hidden ${isOpen ? "max-h-72 border-t border-slate-200" : "max-h-0"}`}>
        <div className="space-y-2 p-4">
          {options.map((option) => (
            <label key={option} className="flex cursor-pointer items-center gap-3 rounded-lg p-2 hover:bg-slate-50">
              <input
                type="checkbox"
